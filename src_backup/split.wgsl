...
fn split_tetra(
    ...
    // Setup internal adjacencies (faces that connect to other new tets)
    set_opp_at(t0, 1u, encode_opp(t1, 0u));   // T0 face 1 <-> T1 face 0
    set_opp_at(t0, 2u, encode_opp(t2, 0u));   // T0 face 2 <-> T2 face 0
    set_opp_at(t0, 3u, encode_opp(t3, 0u));   // T0 face 3 <-> T3 face 0

    set_opp_at(t1, 0u, encode_opp(t0, 1u));   // T1 face 0 <-> T0 face 1
    set_opp_at(t1, 2u, encode_opp(t2, 1u));   // T1 face 2 <-> T2 face 1
    set_opp_at(t1, 3u, encode_opp(t3, 1u));   // T1 face 3 <-> T3 face 1

    set_opp_at(t2, 0u, encode_opp(t0, 2u));   // T2 face 0 <-> T0 face 2
    set_opp_at(t2, 1u, encode_opp(t1, 2u));   // T2 face 1 <-> T1 face 2
    set_opp_at(t2, 3u, encode_opp(t3, 2u));   // T2 face 3 <-> T3 face 2

    set_opp_at(t3, 0u, encode_opp(t0, 3u));   // T3 face 0 <-> T0 face 3
    set_opp_at(t3, 1u, encode_opp(t1, 3u));   // T3 face 1 <-> T1 face 3
    set_opp_at(t3, 2u, encode_opp(t2, 3u));   // T3 face 2 <-> T2 face 3

    // Setup external adjacencies
    // TEMPORARILY DISABLED to isolate crash
    set_opp_at(t0, 0u, orig_opp_0);
    set_opp_at(t1, 1u, orig_opp_1);
    set_opp_at(t2, 2u, orig_opp_2);
    set_opp_at(t3, 3u, orig_opp_3);
